...
public final class No2ArrayExample {
    ...
    public static void main(String[] args) {
        try (NDManager manager = NDManager.newBaseManager()) {
            ...
            System.out.println("现在拥有三个维度\n" + nd.getShape().dimension());

            // 2. 数组的shape
            nd = manager.create(new int[][]{{1, 2, 3}, {4, 5, 6}});
            System.out.println("数组的shape\n" + nd.getShape());

            // 3. 调整数组形状
            nd = nd.reshape(3, 2);
            System.out.println("调整数组形状\n" + nd);

            // 4. 创建数组 zeros
            nd = manager.zeros(new Shape(5), DataType.INT32);
            System.out.println("创建数组 zeros\n" + nd);

            // 5. 创建数组 ones
            nd = manager.ones(new Shape(2, 2), DataType.INT32);
            System.out.println("创建数组 ones\n" + nd);

            // 6. 从数值范围创建数组
            // 6.1 生成 0 到 5 的数组
            nd = manager.arange(5);
            System.out.println("从数值范围创建数组\n" + nd);

            // 6.2 设置返回类型为 float
            nd = manager.arange(0, 5, 1, DataType.FLOAT32);
            System.out.println("设置返回类型为 float\n" + nd);

            // 6.3 设置了起始值、终止值及步长
            nd = manager.arange(10, 20, 2);
            System.out.println("设置了起始值、终止值及步长\n" + nd);

            // 6.4 等差数列 linspace
            nd = manager.linspace(1, 10, 10);
            System.out.println("等差数列 linspace\n" + nd);

            // 7. 数组操作
            // 7.1 生成 0 到 5 的数组
            nd = manager.arange(12).reshape(3, 4);
            System.out.println("生成 0 到 5 的数组\n" + nd);

            nd = nd.transpose();
            System.out.println("创建数组 transpose\n" + nd);

            // 7.2 交换数组的两个轴
            nd = manager.arange(8).reshape(2, 2, 2);
            System.out.println( nd );
            nd = nd.swapAxes(2, 0);
            System.out.println("交换数组的两个轴\n" + nd);

            // 7.3 广播 - 将数组广播到新形状
            nd = manager.arange(4).reshape(1, 4);
            System.out.println(nd);
            nd = nd.broadcast(new Shape(4, 4));
            System.out.println("广播 - 将数组广播到新形状\n" + nd);

            // 7.4 在指定位置插入新的轴来扩展数组形状
            NDArray x = manager.create(new int[][]{{1, 2}, {3, 4}});
            System.out.println("在指定位置插入新的轴来扩展数组形状 数组 x：");
            System.out.println(x);
            // 在位置 0 插入轴
            NDArray y = x.expandDims(0);
            System.out.println("在位置 0 插入轴 y：");
