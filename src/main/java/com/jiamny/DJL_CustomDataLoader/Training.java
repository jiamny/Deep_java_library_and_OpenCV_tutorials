...
public final class Training {
    ...
    public static void main(String[] args) throws IOException, TranslateException {
        ...
        ImageFolder dataset = initDataset("/media/hhj/localssd/DL_data/flower_photos");
        System.out.println(dataset.size());

        // Split the dataset set into training dataset and validate dataset
        RandomAccessDataset[] datasets = dataset.randomSplit(8, 2);
        System.out.println(datasets[0].size());
        ...
        try (Model model = Models.getModel(); // empty model instance to hold patterns
            ...
            Shape inputShape = new Shape(BATCH_SIZE, 3, Models.IMAGE_HEIGHT, Models.IMAGE_HEIGHT);

            // initialize trainer with proper input shape
            trainer.initialize(inputShape);

            for (Batch batch : trainer.iterateDataset(datasets[0])) {
                System.out.println(batch.getData().get(0));
                break;
            }
